(function (c, a, e) {
  ...
  b = function (j, i) {
    var h = (this.$tabs = e(j)),
      k = (this.options = e.extend(true, {}, f, i)),
      g = (this.$nav = h.find(k.navSelector)),
  ...
  b.prototype = {
    init: function () {
      var j = (this.itemsLenth = this.$items.length),
        h;
      if (j === 0) {
      ...
      if (this.$dropdown.length === 0) {
        ...
        this.$nav.append(
          '<li class="dropdown" role="presentation">' +
            ...
            '<span>Xem thêm</span> <b class="caret"></b></a><ul class="dropdown-menu" role="menu"></ul></li>'
        );
    ...
    layout: function () {
      ...
      var l = this.$tabs.width() - 500,
        j = 0,
        h = this,
        g = this.options.activeClassName,
        m = this.$tabPanel.filter("." + g).index(),
        k = function (p) {
          var o = p;
          if (p === h.itemsLenth) {
            o = p - 1;
          ...
          for (; o < h.itemsLenth; o++) {
            if (h.flag) {
              ...
              h.$dropdown
                .find("ul>li" + h.options.noNavClassName + ":first")
                .before(h.$items.eq(o).prop("outerHTML"));
            }
            h.$items.eq(o).hide();
          }
        },
      ...
      if (j === this.breakpoints.length) {
        if (this.flag) {
          ...
          this.$dropdown
            .find("ul>li:not(li" + this.options.noNavClassName + ")")
            .remove();
        }
        this.$items.show();
        if (m != -1) {

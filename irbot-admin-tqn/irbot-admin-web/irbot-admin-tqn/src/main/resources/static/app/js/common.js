    ...
    if ($(".time-input").length > 0) {
        layui.use('laydate', function () {
            ...
            $(".time-input").each(function (index, item) {
                ...
                // Controlling the echo format
                var format = time.attr("data-format") || 'yyyy-MM-dd';
                // Control date control buttons
                var buttons = time.attr("data-btn") || 'clear|now|confirm', newBtnArr = [];
                // Call back after the date control is selected
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        window.parent.$.modal.loading("Đang tải dữ liệu, vui lòng đợi ...");
...
function scrollToTab(element) {
	...
	//The width of the visible area tab
...
function checkpwd(chrtype, password) {
    if (chrtype == 1) {
        if (!$.common.numValid(password)) {
            $.modal.alertWarning("Mật khẩu chỉ có thể có 0-9 chữ số");
            return false;
        }
    } else if (chrtype == 2) {
        if (!$.common.enValid(password)) {
            $.modal.alertWarning("Mật khẩu chỉ có thể là các chữ cái a-z và A-Z");
            return false;
        }
    } else if (chrtype == 3) {
        if (!$.common.enNumValid(password)) {
            $.modal.alertWarning("Mật khẩu phải chứa các chữ cái và số");
            return false;
        }
    } else if (chrtype == 4) {
        if (!$.common.charValid(password)) {
            $.modal.alertWarning("Mật khẩu phải chứa các chữ cái, số và các ký hiệu đặc biệt<font color='red'>~!@#$%^&*()-=_+</font>");
            return false;
        }
    }
    return true;
...
var sub = {
    ...
    delColumn: function (column) {
        ...
        if (ids.length == 0) {
            $.modal.alertWarning("Vui lòng chọn ít nhật 1 dòng");
            return;
        }
...
$.ajaxSetup({
    complete: function (XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
            $.modal.alertWarning("Máy chủ không phản hồi, vui lòng thử lại sau!");
            $.modal.enable();
            $.modal.closeLoading();
        } else if (textStatus == "parsererror" || textStatus == "error") {
            $.modal.alertWarning("Lỗi máy chủ, vui lòng liên hệ quản trị viên!");
            $.modal.enable();
            $.modal.closeLoading();
        }
    }
